...
import me.dvyy.nmr.app.nodes.data.parameters.AnyNodeParameter
import me.dvyy.nmr.io.Formats
...
abstract class Node : AutoCloseable {
    ...
    val id: Int = NodeRepository.nextId()
    ...
    inline fun <reified T> inputAttribute(): InputAttribute<T> {
        return InputAttribute<T>(NodeRepository.nextId(), localId = attributes.size, typeOf<T>()).also { attributes += it }
    }

    inline fun <reified T> outputAttribute(noinline calculation: () -> T): OutputAttribute<T> {
        return OutputAttribute(NodeRepository.nextId(), localId = attributes.size, typeOf<T>(), calculation).also { attributes += it }
    }
