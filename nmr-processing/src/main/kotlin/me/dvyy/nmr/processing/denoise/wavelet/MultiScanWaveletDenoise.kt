package me.dvyy.nmr.processing.denoise.wavelet

import me.dvyy.nmr.bindings.wavelib.StationaryWaveletTransform

object MultiScanWaveletDenoise {
    fun swt(scan: DoubleArray, level: Int): DoubleArray = StationaryWaveletTransform(
        waveletName = "db2",
        signalLength = scan.size,
        level = level
    ).use { swt ->
        swt.forward(scan)
    }

    fun iswt(scan: DoubleArray, level: Int): DoubleArray = StationaryWaveletTransform(
        waveletName = "db2",
        signalLength = scan.size / (level + 1),
        level = level
    ).use { swt ->
        swt.inverse(scan)
    }
